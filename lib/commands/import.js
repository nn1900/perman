...
function restoreRolePermissions(
  ...
  return Promise.resolve(values).then(() => {
    ...
    console.log(sql);
    return provider.query(sql).then(values);
  });
}
...
module.exports = function(config) {
  ...
  return parse(path)
    ...
    .then(result => {
      var sql = result.sql;
      log.info('Dump existing role associated permissions data... ');
      return dumpExistingRolePermissionMap(provider, config)
        .then(existingRolePermissions => {
        log.info('%d role permissions dumpped. ', existingRolePermissions.length);
        result.existingRolePermissions = existingRolePermissions;
        return result;
      });
    })
    .then(result => {
      log.info('Clearing existing role and permission related data... ');
      return clearExistingStuffs(provider, config).then(() => result);
    })
    .then(result => {
      log.info('Importing new permissions definition... ');
      return provider.query(result.sql).then(() => result);
    })
    .then(result => {
      log.info('Restoring existing role permissions... ');
      ...
      return restoreRolePermissions(
        ...
        log.info('%d restored. ', restored.length);
